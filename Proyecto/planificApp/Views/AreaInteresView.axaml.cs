...
public partial class AreaInteresView : UserControl
{
    ...
    private async void AddTaskInput_KeyDown(object? sender, KeyEventArgs e)
    {
        ...
        if (DataContext is not AreaInteresViewModel vm) return;
    ...
    private async void AreaInteresView_Loaded(object? sender, RoutedEventArgs e)
    {
        if (DataContext is AreaInteresViewModel vm)
    ...
    {
        if (DataContext is not AreaInteresViewModel vm || vm.TareaSeleccionada == null) return;
    ...
    private void ShowDetail(AreaInteresViewModel vm)
    {
